...
%{
[row, ~] = find(round(m.NormTime,3) == 10);
if isempty(row)
    initRange = lengthOfWPRecord;
else
    initRange = row;
end
...
beamInitialAngle(1) = mean(m.wpAngles(1:initRange,3)); %Initial angle top of beam
beamInitialAngle(2) = mean(m.wpAngles(1:initRange,6)); %Initial angle bot of beam
beamInitialAngle(3) = mean(m.wpAngles(1:initRange,9)); %Initial angle of pivot rod
beamInitialAngle(4) = mean(m.wpAngles(1:initRange,12)); %Initial angle of column top %%%REMAP
beamInitialAngle(5) = mean(m.wpAngles(1:initRange,14)); %Initial angle top of beam
beamInitialAngle(6) = mean(m.wpAngles(1:initRange,17)); %Initial angle of bot rod

%Compare current angle between sides a & b (angle gamma) to the
%initial angle.
m.beamRotation(:,1) = m.wpAngles(:, 3) - beamInitialAngle(1);
m.beamRotation(:,2) = m.wpAngles(:, 6) - beamInitialAngle(2);
m.beamRotation(:,3) = m.wpAngles(:, 9) - beamInitialAngle(3);
m.beamRotation(:,4) = m.wpAngles(:, 14) - beamInitialAngle(5);
m.beamRotation(:,5) = m.wpAngles(:, 17) - beamInitialAngle(6);
m.beamRotation(:,6) = m.wpAngles(:, 12) - beamInitialAngle(4); %%%REMAP
%}
...
rotInfo3(lengthOfWPRecord) = struct('R', [], 't', [], 's', [], 'M', [], 'theta', []);
