...
%% Filter Manifest Entry Viewer
% ---------------------------------------------------------------------------------------------------------------------------
%
    %% updManDisp() - Updates filter manifest record of selected signal
    % -----------------------------------------------------------------------------------------------------------------------
    function updManDisp(signal)
        % Check that this request is valid
        if ~isfield(filterManifest, signal)
            return
        end
        
        % Shortcut
        sgnl = filterManifest.(signal);
        
        %Turn on relevent UIControls
        setGUIComp({'manifDisp_pnl_off', 'manifDisp_pnl_on'}, {'Visible'}, {'off'; 'on'});
        
        % Get Filter Manifest Entries for selected signal
        manifestValues = {num2str(sgnl.wn); num2str(sgnl.filterOrder); sgnl.filterType; sgnl.filterMethod};
        ...
        setGUIComp({'manifDisp_txt_filtMethVal', 'manifDisp_txt_filtTypeVal', 'manifDisp_txt_filtOrdVal', ...
            'manifDisp_txt_wnVal'}, {'String'}, manifestValues(:));
        
        %Enable Set button & Specify tag
        enableGUIComp('manifDisp_Btn_setDefs');
        setGUIComp('manifDisp_Btn_setDefs', 'Tag', signal)
    end

    %% rstManDisp() - Resets/hides the contents of the filter manifest panel
    % -----------------------------------------------------------------------------------------------------------------------
    function rstManDisp()
        setGUIComp({'manifDisp_pnl_off', 'manifDisp_pnl_on'}, {'Visible'}, {'on'; 'off'});
    end

    %% setDefFiltVals() - Sets the default fir1() FIlter Design values to selected signal's Filter Manifest entry
    % -----------------------------------------------------------------------------------------------------------------------
    function setDefFiltVals(hObject, eventData, handles) %#ok<INUSD,INUSL>
        % Shortcut
        sgnl = filterManifest.(eventData.Source.Tag);
        
        switch sgnl.filterType
            case 'low'
                fir1FilterTypeValue = 1;
            case 'high'
        ...
        fir1FilterType = sgnl.filterType;
        fir1FiltOrd  =   sgnl.filterOrder;
        fir1FiltFreq =   sgnl.wn;
        
        setGUIComp('filtDesg_fir1_filtTypeSet', 'Value', fir1FilterTypeValue);
        setGUIComp({'filtDesg_fir1_filtOrdSet', 'filtDesg_fir1_filtWnSet'}, {'String'}, ...
            {num2str(fir1FiltOrd); num2str(fir1FiltFreq)});
        
        setGUIComp('manifDisp_Btn_clrDefs_off', 'Visible', 'on');
        enableGUIComp({'manifDisp_Btn_clrDefs_on', 'manifDisp_Btn_clrDefs_off'});
    end

    %% clrDefFiltVals() - Clears the default filter values currently set
    % -----------------------------------------------------------------------------------------------------------------------
    function clrDefFiltVals(hObject, eventData, handles) %#ok<INUSD>
        fir1FilterType      = 'low';
        fir1FilterTypeValue = 1;
        fir1FiltOrd         = [];
        fir1FiltFreq        = [];
        ...
        setGUIComp('filtDesg_fir1_filtTypeSet', 'Value', fir1FilterTypeValue);
        setGUIComp({'filtDesg_fir1_filtOrdSet', 'filtDesg_fir1_filtWnSet'}, {'String'}, ...
            {fir1FiltOrd; fir1FiltFreq});
        
        setGUIComp('manifDisp_Btn_clrDefs_off', 'Visible', 'off');
        disableGUIComp({'manifDisp_Btn_clrDefs_on', 'manifDisp_Btn_clrDefs_off'});
    end
...
%
    ...
    function createFilterManifestDisplayPnl()
       ...
       initGUIComp('manifDisp_txt_noManifEnt');
        createLastGUIComp( ...
            'Type', 'uicontrol', ...
            'Parent', filterGUI.manifDisp_pnl_off, ...
            'Style', 'text', ...
            'String', 'No Filter Manifest Entry to display.', ...
            'HorizontalAlignment', 'left', ...
            'Units', 'normalized', ...
            'Position', [0.13055 0.7500 0.7400 0.16]);
        
        initGUIComp('manifDisp_Btn_clrDefs_off');
        createLastGUIComp( ...
            'Type', 'uicontrol', ...
            'Parent', filterGUI.manifDisp_pnl_off, ...
            ...
            'Callback', @clrDefFiltVals);
        
        initGUIComp('manifDisp_pnl_on');
        createLastGUIComp( ...
            'Type', 'uipanel', ...
            'Parent', filterAxes.fig, ...
            'Title', 'Filter Manifest Entry Viewer', ...
        ...
        createLastGUIComp( ...
            'Type', 'uicontrol', ...
            'Parent', filterGUI.manifDisp_pnl_on, ...
            'Style', 'text', ...
        ...
        initGUIComp('manifDisp_Btn_clrDefs_on');
        createLastGUIComp( ...
            'Type', 'uicontrol', ...
            'Parent', filterGUI.manifDisp_pnl_on, ...
            'String', 'Clear Default', ...
